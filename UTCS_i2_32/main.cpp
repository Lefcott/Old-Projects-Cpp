...
void hacer_cambio(char cntrl1[20], char cntrl2[20], char cntrl3[20],
				char cntrl4[20], char cntrl5[20], char cntrl6[20]);
...
void mover_fila(){
	presion = 0;
	while(presion != 32 && presion != 72 && presion != 80){
		presion = getch();
	}
	fila = (fila_nula(fila) == true) ? primera_fila(1) : fila;
...
void mover_ficha(){
	...
	dibujar_tablero();
	presion = 0;
	while(presion != 32 && presion != 13 && presion != 75 && presion != 77){
		presion = getch();
	}
...
void funcion_controles(int opcion){
	...
	if(opcion == 2){
		hacer_cambio("IZQUIERDA", "DERECHA", "ARRIBA", "ABAJO", "EFECTUAR CAMBIOS", "SELECCIONAR FILA/FICHA");
	}
	menu();
}
void mostrar_controles(){
	for(int i = 0; i < 6; i++){
		if(controles[i] == 75 || controles[i] == 77 || controles[i] == 72 || controles[i] == 80 || controles[i] == 13 || controles[i] == 32){
			printf("%s:\n\t%s\n", (i == 0) ? "IZQUIERDA" : (i == 1) ? "DERECHA" : (i == 2) ? "ARRIBA" : (i == 3) ? "ABAJO" : (i == 4) ? "EFECTUAR CAMBIOS" : (i == 5) ? "SELECCIONAR FILA/FICHA" : "OTRO", (controles[i] == 75) ? "Flecha izquierda" : (controles[i] == 77) ? "Flecha derecha" : (controles[i] == 72) ? "Flecha arriba" : (controles[i] == 80) ? "Flecha abajo" : (controles[i] == 13) ? "Enter" : (controles[i] == 32) ? "Espacio" : "OTRA");
		}
		else{
			printf("%s:\n\t%c\n", (i == 0) ? "IZQUIERDA" : (i == 1) ? "DERECHA" : (i == 2) ? "ARRIBA" : (i == 3) ? "ABAJO" : (i == 4) ? "EFECTUAR CAMBIOS" : (i == 5) ? "SELECCIONAR FILA/FICHA" : "OTRO", controles[i]);
		}
	}
...
void hacer_cambio(char cntrl1[20], char cntrl2[20], char cntrl3[20], char cntrl4[20], char cntrl5[20], char cntrl6[20]){
	for(int i = 0; i < 6; i++){
		printf("%s:\n", (i == 0) ? cntrl1 : (i == 1) ? cntrl2 :(i == 2) ? cntrl3 :(i == 3) ? cntrl4 :(i == 4) ? cntrl5 :(i == 5) ? cntrl6 : "ERROR");
		controles[i] = getch();
		controles[i] = getch();
